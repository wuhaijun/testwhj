...
(function (factory) {
  ...
  var dom = (function () {
    ...
    return {
      ...
      /** @property {String} emptyPara */
         ...
         */
      emptyPara: '<p style="font-size: 14px;">' + blankHTML + '</p>',
  ...
  var codable = renderer.create('<textarea class="note-codable"/>');
     ...
     *
     */
  var editable = renderer.create('<section id="editor.brainboom" class="note-editable panel-body" contentEditable="true"/>');
  ...
  var airEditor = renderer.create('<div class="note-editor"/>');
     ...
     */
  var airEditable = renderer.create('<section id="editor.brainboom" class="note-editable" contentEditable="true"/>');
    ...
    var WrappedRange = function (sc, so, ec, eo) {
      ...
      this.insertNode = function (node) {
        ...
        var info = dom.splitPoint(rng.getStartPoint(), dom.isInline(node));
           ...
           *
           */
          // console.log({infoRightNodeOuterHTML:info.rightNode.outerHTML,infoContainerOuterHTML:info.container.outerHTML})
          if (info.rightNode && info.rightNode.outerHTML == dom.blank &&
            info.container && info.container.outerHTML == dom.emptyPara) {
          info.container.parentNode.insertBefore(node, info.container);
          return node;
        }

        /***
           * winter.editor
           * 判断是否为code，解决code中按tab换行
           *!($(info.rightNode.outerHTML).is('code'))
           */
        if (info.rightNode ) {
  ...
  var History = function ($editable) {
    ...
    var max = (window.MAX_UNDO_STACK_LENGTH || 500);  //winter.editor 新加的语句
    var stack = [], stackOffset = -1;
    var editable = $editable[0];

    /* @hack  winter.editor  以下是新添加的代码
      此方法已经被realHtml取代，未避免在否些无法预料的情况下，还没有注册realHtml方法时被调用而报错。
      故增加判断，原单纯过滤active的代码保留。
    */
    var editorHtmlWithoutActiveClass = function () {
      if($editable.realHtml) {
        return $editable.realHtml();
      }else {
        var temp = $editable.clone();
        temp.children('.winter-section-active').removeClass('winter-section-active');
        return temp.html();
      }
    };
    ...
    var makeSnapshot = function () {
      ...
      return {
        contents: editorHtmlWithoutActiveClass(),
        sectionIndex: $editable.children('.winter-section-active').index(),  //winter.editor  新加语句
    ...
    var applySnapshot = function (snapshot) {
      if (snapshot.contents !== null) {
         ...
         */
      //if (snapshot.bookmark !== null) {
      //  var rw = range.createFromBookmark(editable, snapshot.bookmark);
      //  rw.select();
      //}
    };
    ...
    */
    this.rewind = function () {
      // Create snap shot if not yet recorded
         ...
         *
         */
      if (editorHtmlWithoutActiveClass() !== stack[stackOffset].contents) {
        this.recordUndo();
      }
    ...
    */
    ...
    this.recordUndo = function () {
         ...
         */
      if (stackOffset>=0 &&
          editorHtmlWithoutActiveClass() === stack[stackOffset].contents) {
          return;
      }
      ...
      if (stack.length > stackOffset) {
         ...
         */
      if (stackOffset > max) {
        stack = stack.slice(stackOffset - max, stackOffset);
        stackOffset = max;
      }
     ...
     *  新加方法
     */
    this.navy_status = function () {
      return { undo: stackOffset == 0 ? "true" : "false",redo: stack.length-1 > stackOffset ? "false" : "true"};
    }
  };
  ...
  var Style = function () {
     ...
     *  var properties = ['font-family', 'font-size', 'text-align', 'list-style-type', 'line-height', 'letter-spacing','margin-top','margin-bottom','margin-left','text-indent'];
     *   var styleInfo = jQueryCSS($node, properties) || {};
     *   styleInfo['font-size'] = parseInt(styleInfo['font-size'], 10) || 14;
     *  styleInfo['letter-spacing'] = parseInt(styleInfo['letter-spacing'], 10);
     *   styleInfo['margin-top'] = parseInt(styleInfo['margin-top'], 10);
     *   styleInfo['margin-bottom'] = parseInt(styleInfo['margin-bottom'], 10);
     *   styleInfo['margin-left'] = parseInt(styleInfo['margin-left'], 10);
     *   return styleInfo;
     *   };
     *
     */
  ...
  var Bullet = function () {
      ...
      $.each(clustereds, function (idx, paras) {
        ...
        if (dom.isLi(head)) {
          ...
          $.each(paras, function (idx, para) {
            ...
            //});
            $(para).css('textIndent', function (idx, val) {
     ...
     *  rowspacingtop
     */
    this.rowspacingtop = function (editable,rowspacingtop) {
      var self = this;
      var rng = range.create(editable).wrapBodyInlineWithPara();
      ...
      $.each(clustereds, function (idx, paras) {
        ...
        if (dom.isLi(head)) {
          ...
          $.each(paras, function (idx, para) {
            $(para).css('marginTop', function (idx, val) {
                return parseInt(rowspacingtop);
            });
          });
        }
      });
     ...
     * rowspacingbottom
     */
    this.rowspacingbottom = function (editable,rowspacingbottom) {
      var self = this;
      var rng = range.create(editable).wrapBodyInlineWithPara();
      ...
      $.each(clustereds, function (idx, paras) {
        ...
        if (dom.isLi(head)) {
          ...
          $.each(paras, function (idx, para) {
            $(para).css('marginBottom', function (idx, val) {
              return parseInt(rowspacingbottom);
            });
          });
        }
      });
     ...
     * betweenSpace
     */
    this.betweenSpace = function (editable,betweenSpace) {
      var self = this;
      var rng = range.create(editable).wrapBodyInlineWithPara();
      ...
      $.each(clustereds, function (idx, paras) {
        ...
        if (dom.isLi(head)) {
          ...
          $.each(paras, function (idx, para) {
            $(para).css('marginLeft', function (idx, val) {
              return parseInt(betweenSpace);
            });
            $(para).css('marginRight', function (idx, val) {
              return parseInt(betweenSpace);
            });
          });
        }
     ...
     * 自定义去格式
    */
    this.customRemoveFormat = function (editable) {
      var self = this;
      var html = "";
      ...
      if (typeof window.getSelection != "undefined") {
          ...
          if (sel.rangeCount) {
              ...
              for (var i = 0, len = sel.rangeCount; i < len; ++i) {
                  var contents = sel.getRangeAt(i).extractContents();
                  container.appendChild(contents);
              }
              html = container.innerHTML;
              var result = $("<section class='addition'>"+html+"</section>")
              __filterHtml__(result);
              sel.getRangeAt(0).insertNode(result[0])
          }
      }
       //递归传入的dom 去除style
         function __filterHtml__ ($dom) {
          __cleanHtml__($dom);
          if ($dom.children().length > 0) {
              $dom.children().each(function (index, child) {
                  __filterHtml__($(child));
              });
          }
      }

        //去除传入的dom style
        function __cleanHtml__($dom){
  ...
  var Typing = function () {
    ...
    this.insertParagraph = function (editable) {
      ...
      //winter.editor
      var nodes = rng.nodes();
      if($(nodes).hasClass("winter-section-inner")) {
        $(nextPara).children().each(function(){
          $(this).removeClass("winter-section-active winter-section-p");
        });
      }
      ...
      // 回车换行后处理前一行数据
      __handlePrevAfterInsertParagraph__($(nextPara).prev());
    };


    var ctx = this;
    var __handlePrevAfterInsertParagraph__ = function($prev) {
      if ($prev.length == 0) return;
      var text = $prev.text();

      // 行首如果是 #+ 开头,回车将此行设置为标题
      var titlePattern = new RegExp('^(#+)\\s+.+', 'i');
      var match = titlePattern.exec(text);
      if (match) {
        $prev.html(text.replace(/^#+\s+/i, '') || '<br/>');
        var titlePrefix = match[1];
        if (titlePrefix.length == 1) {
          $prev.css('font-size', '24px').css('font-weight', 'bold');
        } else if (titlePrefix.length == 2) {
          $prev.css('font-size', '18px').css('font-weight', 'bold');
        } else if (titlePrefix.length == 3) {
          $prev.css('font-size', '16px').css('font-weight', 'bold');
        } else if (titlePrefix.length >= 4) {
          $prev.css('font-size', '14px').css('font-weight', 'initial');
        }
        return;
      }

      // 行首如果是 >+ 开头,回车将此行设置为引用
      var blockquotePattern = new RegExp('^>+\\s+', 'i');
      if (blockquotePattern.test(text)) {
        $prev.html(text.replace(/^>+\s+/i, '') || '<br/>');
        $prev.wrap("<blockquote style='padding: 10px 15px; border-left: 10px solid #D6DBDF;background: none repeat scroll 0 0 rgba(102,128,153,.05);'></blockquote>");
          return;
      }

      // 如果整行只有* 和-,回车将此行设置为分割线
      var splitLinePattern = new RegExp('^[\*\-]+$', 'i');
      if (splitLinePattern.test(text)) {
        $prev.html('<hr/>');
        return;
      }

      // 如果如果整行只有``` 回车将此行设置为代码块
      var codePattern = new RegExp('^```\\s?', 'i');
      if (codePattern.test(text)) {
        var content = text.replace(/^```\s?/i, '');

        // 代码区域
        var $code = $('<code tabindex="0" class="java hljs"></code>');
        $code.html(content ? "<p>"+content+"</p>" : dom.emptyPara);

        // 语言选择下拉列表
        var $languageSelector = $(
        '<section class="language-selector" style="display: none">'+
        ...
        // 样式选择下拉列表
        var $styleSelector = $(
          '<section class="style-selector" style="display: none">'+
          ' <select name="css">'+
        ...
        var $pre = $('<pre class="winter-section-code"></pre>').css({
          position: "relative",
          padding: '2px'
        });
        $pre.append($code);
        ...
        ctx.__codeHandler__($pre);
        ctx.__codeRender__($pre);
    ...
    var __optimizeCode__ = function($code) {
      $code.find('span[class^="hljs-"]').each(function() {
        $(this).replaceWith(this.childNodes);
      })
    };

    var __setCodeCss__ = function($pre, style) {
      var css = style == 'dark' ? {
    ...
    this.__codeHandler__ = function($pre) {

      var $languageSelector = $pre.find('.language-selector');
      var $styleSelector = $pre.find('.style-selector');

      $pre.each(function(index,dom){

          $($languageSelector[index]).find('select').on('change', function() {
              $(dom).trigger('editor.code.render');
          });

          $($styleSelector[index]).find('select').on('change', function() {
              $(dom).trigger('editor.code.render');
          });
      });
    };

    this.__codeRender__ = function($pre) {

        $pre.each(function(index,dom){
            $(dom).on('editor.code.render', function() {
               var $this = $(this);
               var $languageSelector = $this.find('.language-selector');
               var $styleSelector = $this.find('.style-selector');
               ...
               __optimizeCode__($code);
               hljs.highlightBlock($code[0]);
               __setCodeCss__($(dom), style);
               var codeStyle = $code.attr('style');
               $code.attr('style',codeStyle + "white-space: nowrap;");
           });
       });
  ...
  /**
   * @class Crture a virtual table to create what actions to do in change.
  ...
  */
  ...
  var Editor = function (context) {
    ...
    this.saveRange = function (thenCollapse) {
         ...
         *
         */
      lastRange = range.create(editable);
     ...
     *
     */
    this.historyStatus = function () {
      return history.navy_status();
    };
    ...
    this.__codeHandler__= this.wrapCommand(function ($code) {
      typing.__codeHandler__($code);
    });
    this.__codeRender__= this.wrapCommand(function ($code) {
      typing.__codeRender__($code);
    });
    ...
    context.memo('help.outdent', lang.help.outdent);
       ...
       *
       */
    this.rowspacingtop = this.wrapCommand(function (rowspacingtop) {
      bullet.rowspacingtop(editable,rowspacingtop);
    });
    context.memo('help.rowspacingtop', lang.help.rowspacingtop);

    this.rowspacingbottom = this.wrapCommand(function (rowspacingbottom) {
      bullet.rowspacingbottom(editable,rowspacingbottom);
    });
    context.memo('help.rowspacingbottom', lang.help.rowspacingbottom);

    this.betweenSpace = this.wrapCommand(function (betweenSpace) {
      bullet.betweenSpace(editable,betweenSpace);
    });
    context.memo('help.betweenSpace', lang.help.betweenSpace);

    this.customRemoveFormat = this.wrapCommand(function (){
      bullet.customRemoveFormat(editable);
    });
    context.memo('help.customRemoveFormat',lang.help.customRemoveFormat);
    ...
    this.insertImage = function (src, param) {
      return async.createImage(src, param).then(function ($image) {
        ...
        if (typeof param === 'function') {
          ...
          if (typeof param === 'string') {
             ...
             *
             */
          if(window.defaultInsertType){
            $image.css({"width":"100%","vertical-align":"middle","margin-top":"-1px"}).addClass("seamlessImage");
          }else{
            $image.css({"width":"100%"});
          }
        ...
        $image.show();
           ...
           */
        var $inserted = self.__createSection__($image);
        var rng = range.create(editable);
        var nodes = rng.nodes();
        if ( !window.defaultInsertType || !nodes[0] || !nodes[0].classList ||  !nodes[0].classList.contains('winter-section-p')) {
            rng.insertNode($inserted[0]);
            range.createFromNodeAfter($inserted[0]).select();

        } else {
            nodes[0].firstChild && nodes[0].firstChild.appendChild($image[0]);
        }
       ...
       * @private
       */
    this.__createSection__ = function($node) {
          var $section = $("<section class='winter-section-p' powered-by='editor.brainboom'></section>")
              .css({
                  'margin-bottom': '0px'
              });

          var $section_inner = $('<section class="winter-section-inner"></section>')
              .css({
                  'display': 'block',
                  'clear': 'both',
                  'overflow': 'hidden',
      ...
      if($node.hasClass('dynamic-section')) {
        $section_inner.addClass('winter-section-dynamic');
      }
      if($node.hasClass('slide-style')) {
        $section_inner.addClass('winter-section-slide');
      }
    ...
    //winter.editor 自定义引用方法
    this.__formatBlock__ = this.wrapCommand(function (tagName) {
        this.onFormatBlock(tagName);
        var selection = window.getSelection();
        var current = selection.isCollapsed ? selection.focusNode : selection.focusNode.parentNode;
        $(current).css({
            padding: "10px 15px",
    ...
    this.createLink = this.wrapCommand(function (linkInfo) {
      ...
      var anchors = [];
      // if (isTextChanged) {  源码
      // winter.editor 判断有问题，当选区有图片时，会把图片删除，替换成文字。
      if (rng.toString() && isTextChanged) {
        rng = rng.deleteContents();
  ...
  var Dropzone = function (context) {
    ...
    this.initialize = function () {
      if (options.disableDragAndDrop) {
        ...
        // winter.editor
       // $document.on('drop', documentEventHandlers.onDrop);  //先不让拖拽
      } else {
        //this.attachDragAndDropEvent();  //先不让拖拽
      }
    };
  ...
  var Handle = function (context) {
    ...
    this.initialize = function () {
      this.$handle = $([
        '<div class="note-handle" style="display: none">',  //winter.editor display:block
  ...
  var Buttons = function (context) {
    ...
    this.updateCurrentStyle = function ($container) {
      ...
      if (styleInfo['letterspacing']) {
        var letterspacing = styleInfo['letterspacing'];
        $cont.find('.dropdown-letterspacing li a').each(function () {
          // always compare with string to avoid creating another func.
          var isChecked = ($(this).data('value') + '') === (letterspacing + '');
          this.className = isChecked ? 'checked' : '';
        });
      }
    ...
    this.initialize = function () {
      ...
      var body = '<div class="form-group note-form-group">' +
                 ...
                 '<div class="form-group note-form-group">' +
                   ...
                   'note-input" type="text" placeholder="http://" value="http://" />' +
                 '</div>' +
      (!options.disableLinkTarget ?
         ...
         * @type {string}
         */
      var footer ='<span style="float:left;color:#666;">注：认证过并开通微信支付的服务号，可放置微信以外的超链接</span>'+'<button href="#" class="btn btn-primary note-btn note-btn-primary ' +
      'note-link-btn " >' + "确定" + '</button>';
//disabled
    ...
    this.update = function () {
      ...
      if (styleInfo.range && !styleInfo.range.isCollapsed()) {
        ...
        if (rect) {
          ...
          this.$popover.css({
            display: 'none',  //winter.editor  display:block
    ...
    options: {
      ...
      icons: {
        ...
        'italic': 'note-icon-italic',
          //winter.editor
        'link': 'fa fa-pencil', //note-icon-link
        'unlink': 'fa fa-trash-o', //note-icon-chain-broken
